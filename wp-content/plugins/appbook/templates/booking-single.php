...
<div id="booking-single">
	...
	<div class="page-header row">
		<h2>
			<?php echo __('Ajouter un rendez-vous', $app->slug);
			// else
	...
	<ul class="nav nav-tabs" role="tablist">
	    <li role="presentation" class="active"><a href="#display-single" aria-controls="display-single" role="tab" data-toggle="tab"><?php _e('Visualiser', $app->slug) ?></a></li>
	    <li role="presentation"><a href="#edit-single" aria-controls="edit-single" role="tab" data-toggle="tab"><?php _e('Modifier', $app->slug) ?></a></li>
  	</ul>
  	<div class="tab-content">
		<div role="tabpanel" class="tab-pane active" id="display-single">
			<table class="no-border-table">
				<tr>
					<td></td>
					<td><h4 class="pull-left"><?php _e('Client', $app->slug) ?></h4></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Prénom', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo stripslashes($current->firstname) ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Nom', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo stripslashes($current->lastname) ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Email', $app->slug) ?></a></span></td>
					<td><span class="pull-left">: <a href="mailto:<?php echo stripslashes($current->email) ?>"><?php echo stripslashes($current->email) ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Téléphone', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo stripslashes($current->phonenumber) ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Message', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo stripslashes($current->message) ?></span></td>
				</tr>
				<tr>
					<td></td>
					<td><h4 class="pull-left"><?php _e('Détails du rendez-vous', $app->slug) ?></h4></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Service', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo stripslashes($app->app->service->getServiceName($current->service_id)) ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Employé(e)', $app->slug) ?></span></td>
					<?php $single_employee = $app->app->employee->getSingle($current->employee_id) ?>
					<td><span class="pull-left">: <?php echo $single_employee->firstname.' '.$single_employee->lastname ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Date', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo date_i18n( 'l j F Y', strtotime( $current->date) ) ?></span></td>
				</tr>
				<tr>
					<td><span class="pull-right"><?php echo __('Heure', $app->slug) ?></span></td>
					<td><span class="pull-left">: <?php echo $current->hour ?></span></td>
				</tr>
			</table>
		</div>
		<div role="tabpanel" class="tab-pane" id="edit-single">
	<?php endif; ?>
			<form id="<?php echo $app->slug; ?>_app<?php echo '_'.$form_action?>"  class="<?php echo $app->slug; ?>_form form-horizontal col-xs-12 col-sm-8" method="POST" action="<?php echo $action ?>">
				...
				<div class="form-group">
			  	...
			  	if (isset($current))
			  	{
			  		$employee_options = '<option value="0"></option>';
...
<script type="text/javascript">
	...
	function getPeriode(hideAlert, onLoad, content){
		...
		var urlWithEmployee = '<?php echo site_url('/getperoides.php') ?>';
		var urlWithoutEmployee = '<?php echo site_url('/getservicesperoides.php') ?>';
		var employeeId = jQuery('#employee_id').val();
		if(employeeId == 0)
		{
			ajaxUrl = urlWithoutEmployee;
		}else{
			ajaxUrl = urlWithEmployee;
		}
		jQuery.post(
			ajaxUrl,
			{
